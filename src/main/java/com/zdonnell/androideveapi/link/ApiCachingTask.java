...
public abstract class ApiCachingTask<ExecuteParameter, ProgressParameter, Response extends ApiResponse> extends ApiTask<ExecuteParameter, ProgressParameter, Response> {
	/**
	 * True if the {@link CacheDatabase} has an entry for the given Response
	 * hash and {@link #useCache} is true.
	 */
	private boolean cacheExists = false;

	/**
	 * True if the {@link CacheDatabase} has an entry for the given Response
	 * hash that has not yet expired and {@link #useCache} is true.
	 */
	...
	protected Response doInBackground(ExecuteParameter... params) {
		...
		if(cacheValid) {
			...
			// The cache is out of date (invalid) but load it anyway while we
	...
	protected void onPostExecute(Response response) {
		// We can arrive here one of two ways, if the cache was still valid, or
		// if it was invalid and a server response was acquired, check which it
		// is.
		if(cacheValid) {
